...
class DataGen(object):
    ...
    def resize_data(self, image):
        h, w = image.shape
        aspect_ratio = float(w) / float(h)
        if aspect_ratio < float(self.bucket_min_width) / self.image_height:
            image = cv2.resize(image, (self.bucket_min_width, self.image_height))
        elif aspect_ratio > float(self.bucket_max_width) / self.image_height:
            image = cv2.resize(image, (self.bucket_max_width, self.image_height))
        elif h != self.image_height:
            image = cv2.resize(image,(int(aspect_ratio * self.image_height), self.image_height))
        #print(image.shape)
